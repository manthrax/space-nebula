...
export default class PlanetGenerator {
    constructor(renderer) {
        ...
        this.material = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    // Map UV to spherical coordinates for equirectangular projection
                    ...
                    vec3 p = spherePos + vec3(uSeed * 0.13, uSeed * 0.47, uSeed * 0.91);
                    float h = 0.0;
                    float amp = 0.5;
                    float freq = 2.0;
                    for(int i=0; i<6; i++) {
                    ...
                    vec3 col;
                    if (h < 0.0) {
                        col = mix(uColorWater * 0.5, uColorWater, h + 1.0);
                    } else if (h < 0.4) {
                        col = mix(uColorLand, uColorLand * 1.2, h / 0.4);
                    } else {
                        col = mix(uColorLand * 1.2, uColorMountain, (h - 0.4) / 0.6);
                    }
    ...
    generate(seed = Math.random(), options = {}) {
        const resolution = options.resolution || 1024;
        const renderTarget = new THREE.WebGLRenderTarget(resolution, resolution * 0.5, {
        ...
        if (options.colorWater) this.material.uniforms.uColorWater.value.set(options.colorWater);
        if (options.colorLand) this.material.uniforms.uColorLand.value.set(options.colorLand);
